...
import h5py
from filters.cosmic import find_cosmics
...
class Mapper:
    ...
    def __init__(
        ...
        self.df_maximum = pd.DataFrame(index=self._midx, columns=["Intensity"])
        self.df_integrated = pd.DataFrame(index=self._midx, columns=["Intensity"])
        self.df_wno = pd.DataFrame(index=self._midx, columns=["Intensity"])
    ...
    def plot(self):
        self.specx_slider = widgets.FloatSlider(
            value=self.specx,
        ...
        self.dspecx_slider = widgets.FloatSlider(
            value=self.dspecx,
            min=0.2,
        ...
        self.recalc_checkbox = widgets.Checkbox(
            value=False, description="Auto", disabled=False
        )

        self.autoscale_button = widgets.Button(
            description="Rescale z-Axis",
            disabled=False,
            layout=Layout(width="75%"),
            tooltip="Autoscale 2D Graph z-Axis",
        )

        self.autoscale_checkbox = widgets.Checkbox(
            value=True, description="Auto", disabled=False
        )

        self.background_button = widgets.Button(
            description="Select background point",
            disabled=False,
            layout=Layout(width="75%"),
            tooltip="Use current data point for background correction",
        )

        self.mask_button = widgets.Button(
            description="Un/mask",
            disabled=False,
            layout=Layout(width="75%"),
        ...
        self.level_slider = widgets.IntSlider(
            value=self.levels, min=3, max=50, description="Lvls", disabled=False
        )
        ...
        self.i_max_slider = widgets.FloatSlider(
            value=self.i_max,
            min=self.i_min,
            max=self.i_max,
        ...
        self.i_min_slider = widgets.FloatSlider(
            value=self.i_min,
            min=self.i_min,
            max=self.i_max,
            step=(self.i_max - self.i_min) / 50,
        ...
        self.c = go.Contour(
            z=Mapper.reshape_df(self.active_df),
            x=self.x,
            y=self.y,
            colorscale="Viridis",
            colorbar=dict(x=-0.2, len=1),
            contours=dict(
                start=float(self.i_min),
        ...
        self.g.data[1].name = "x={:.2f},y={:.2f}".format(
    ...
    def update_graphics(self, change=None):
        df_roi = self.get_roi()
        xpos = Mapper.get_pos_of_max(
            pd.to_numeric(
                df_roi.loc[self.specx - self.dspecx : self.specx + self.dspecx]
            )
        )
        ypos = Mapper.get_intensity(
            df_roi.loc[self.specx - self.dspecx : self.specx + self.dspecx]
        )
        ...
        with self.g.batch_update():
            ...
            self.g.data[0].contours = dict(
                start=float(self.i_min_slider.value),
                end=float(self.i_max_slider.value),
                size=float(self.i_max_slider.value - self.i_min_slider.value)
                / self.levels,
            )
            ...
            self.g.update_shapes(
                patch=dict(x0=self.specx - self.dspecx, x1=self.specx - self.dspecx),
    ...
    def _calculate_2d(self):
        if self.background_select.value == "Mean":
            ...
            self.corrected_df = (
                self.df - self.df.loc[self.bg_point[0], self.bg_point[1]]
            )
        elif self.background_select.value == "y-Mean":
            self.corrected_df = self.df.copy()
            for i in self.x:
                self.corrected_df.loc[(pd.IndexSlice[i, :]), :] = (
                    self.df.loc[(pd.IndexSlice[i, :]), :]
                    - self.df.loc[(pd.IndexSlice[i, :]), :].mean()
                )
        ...
        roi = self.corrected_df.loc[
            :, self.specx - self.dspecx : self.specx + self.dspecx
        ]
        ...
        if self._is_interp:
            ...
            self.df_wno = roi.apply(
                lambda x: pd.to_numeric(x).idxmax(), axis=1
            ).sort_index()
        ...
        if self._is_interp:
            ...
            idx_pos = np.array(
                list(
                    map(
                        lambda c: np.sqrt(
                            (c[0] - self.selected_point[0]) ** 2
                            + (c[1] - self.selected_point[1]) ** 2
                        ),
                        multidx.to_numpy(),
                    )
                )
    ...
    def save_to_files(self, base_filename, append_info=False, unstacked=False):
        info_string = "_cwave={:.2f}_dcwave={:.2f}".format(self.specx, self.dspecx)
        if append_info:
            base_filename += info_string

        if unstacked:
            self._apply_mask(self.df_maximum).unstack().to_csv(
                base_filename + "_maximum.csv"
            )
            self._apply_mask(self.df_integrated).unstack().to_csv(
                base_filename + "_integrated.csv"
            )
            self._apply_mask(self.df_wno).unstack().to_csv(
                base_filename + "_wavelength.csv"
            )
        else:
            self._apply_mask(self.df_maximum).to_csv(base_filename + "_maximum.csv")
            self._apply_mask(self.df_integrated).to_csv(
                base_filename + "_integrated.csv"
            )
    ...
    def save_current_spectrum_to_file(self, filename):
        ...
        pd.DataFrame({"Raw" + pos: df_roi.values}).set_index(df_roi.index).to_csv(
            filename
        )

    def reverse(self, revx, revy):
        self.df_integrated.index = pd.MultiIndex.from_arrays(
            [
                Mapper.get_xindex(self.df_integrated)[:: 1 - 2 * int(revx)],
                Mapper.get_yindex(self.df_integrated)[:: 1 - 2 * int(revy)],
            ]
        )

        self.df_wno.index = pd.MultiIndex.from_arrays(
            [
                Mapper.get_xindex(self.df_wno)[:: 1 - 2 * int(revx)],
    ...
    def als_remove_baseline(self, change=None):
        baseline_i = self.df.apply(
            lambda x: Mapper.baseline_als_optimized(list(x)), axis=1
        )
